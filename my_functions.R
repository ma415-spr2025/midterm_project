drop_one_value_col <- function(df, prt_val = FALSE){ 
  ...
  for(i in seq_along(df)){   
    unique_vals <- unique(df[[i]])  # Get unique values in the column
    
    if(length(unique_vals) == 1){  # Check if there is only one unique value
      dropc <- c(dropc, i)
      val <- c(val, unique_vals[1])
    }
  } 
  ...
  if(!is.null(dropc)) {
    df <- df[, -dropc, drop = FALSE]  # Drop columns with only one unique value
  }
